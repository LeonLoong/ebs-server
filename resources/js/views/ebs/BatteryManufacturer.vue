<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="batteryManufacturers" border fit highlight-current-row style="width: 100%">
...
import { getBatteryManufacturers } from '@/api/ebs';
  ...
  methods: {
    async getData() {
      getBatteryManufacturers(this.query)
        .then(response => {
          ...
          this.batteryManufacturers = data;
          this.batteryManufacturers.forEach((element, index) => {
    ...
    handleExport() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const data = this.formatJson(filterVal, this.batteryManufacturers);
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: 'batteryManufacturers',
        });
